...
import { AvailablePosition } from "@/types/society";
...
export default function ApplyJob() {
  ...
  const [filteredJobs, setFilteredJobs] = useState<AvailablePosition[]>([]);
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      if (!token) return;
      try {
        const response = await applyPositionApi.getAllJobAvailable(token);
    ...
    <ProtectedRoute allowedRoles={["Society"]}>
      <div className="flex flex-col justify-center mt-20 text-xl">
        ...
        <header className="text-center">
          <p>
            <span className="text-orange-600 font-black">—</span> Find Jobs
          </p>
          ...
          <p className="mt-2">
            {" "}
            Explore active job openings that match your skills.
          </p>
        </header>
                      ...
                      {job.submission_start_date && (
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            ...
                            {formatDate(
                              job.submission_end_date || job.submission_end_date
                            )}
                          </span>
                        </div>
                      )}
